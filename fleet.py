...
#
# 2026-08-25 的抽层把这个定义删了、把下面的引用留下了，于是 `keep_marks` 在
# **正文超过 limit 时**抛 NameError。它被 `beat_fleet` 的 `except Exception`
# 接住并记成「beat 失败（不影响收尾）」—— 所以不是丢标记，是**整条心跳都没了**，
# 而且只在长正文时发生。3 天 230 次，没人看见。
# 教训：把「别让它静默失灵」的那段代码删掉时，它自己不会喊。
