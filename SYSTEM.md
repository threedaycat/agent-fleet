...
```bash
python3 dash.py                    # 启动序列（各项逐个点亮）+ 常驻自刷，q 退出
python3 dash.py --once --plain     # 画一帧就退出，可以管道出去
python3 dash.py --fast             # 不采上下文占用（省掉每个 pane 一次 capture）
```

跟 `console.py` **看同一份东西**，只是不用切到浏览器：自检、服务、采集痕迹、
待拍板草稿、哨兵积压、各会话上下文占用（超过 `AUTOCOMPACT_THRESHOLD_KB`
标「该压了」，跟 autocompact 真正用的是同一个常量）。

**判定一律不在 dash.py 里重算**——全部走 `console.collect_*` 和 `fleet_up.svc_status`。
要加指标就加在 `console.py` 的采集函数里，两块屏同时得到；在 dash 里自己算一遍
就有了两份真相。

刷新和动画是两个频率：采集每 2 秒一次（会跑 tmux capture，是最慢的一步），
画面每 0.1 秒一次（纯计算，让进度条平滑追上新值）。

三个地方故意做得「不好看」，因为它们各自防一种谎：

- 上下文读不出时进度条画 `╌╌╌` 而不是空槽——**0% 是个合法值，「不知道」不是**；
- 任何一段采集抛异常，那一段显示「读不出」，不兜成 `0 条`；
- 只显示占用最高的 6 个 pane，但**被省掉几个必须写出来**，否则等于谎报「就这些」。
